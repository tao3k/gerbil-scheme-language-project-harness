"""Bazel projection of the checked POO Flow component source closure."""

# Generated from poo-flow.json. The JSON manifest remains the semantic owner.
POO_FLOW_COMPONENT_SOURCES = [
    "src/build-api/framework.ss",
    "src/build-api/source-coverage.ss",
    "src/building/commands.ss",
    "src/building/facade.ss",
    "src/building/model.ss",
    "src/building/native-toolchain.ss",
    "src/building/observability.ss",
    "src/building/std-builder.ss",
    "src/constants.ss",
    "src/extensions/poo-pattern-support.ss",
    "src/extensions/poo-pattern-typeclass.ss",
    "src/extensions/poo-patterns.ss",
    "src/parser/comment-quality-classifier.ss",
    "src/parser/comment-quality.ss",
    "src/parser/control-flow.ss",
    "src/parser/core.ss",
    "src/parser/dependency-adapter-quality.ss",
    "src/parser/exports.ss",
    "src/parser/facade.ss",
    "src/parser/formals.ss",
    "src/parser/function-quality.ss",
    "src/parser/higher-order.ss",
    "src/parser/imports.ss",
    "src/parser/model.ss",
    "src/parser/package.ss",
    "src/parser/parse-workers.ss",
    "src/parser/poo.ss",
    "src/parser/profile.ss",
    "src/parser/quality-shape.ss",
    "src/parser/reader.ss",
    "src/parser/runtime-contract.ss",
    "src/parser/selectors.ss",
    "src/parser/source-class.ss",
    "src/parser/source-file.ss",
    "src/parser/source-scope.ss",
    "src/parser/support.ss",
    "src/parser/syntax-calls.ss",
    "src/parser/syntax-support.ss",
    "src/parser/syntax.ss",
    "src/parser/test-source-scope.ss",
    "src/parser/typed-comment-metadata.ss",
    "src/parser/typed-contract-diagnostics.ss",
    "src/parser/typed-contract-scheme.ss",
    "src/parser/typed-contract-token.ss",
    "src/parser/typed-contract.ss",
    "src/policy/agent-alist-access.ss",
    "src/policy/agent-anonymous-pair.ss",
    "src/policy/agent-basic.ss",
    "src/policy/agent-build-runtime.ss",
    "src/policy/agent-build.ss",
    "src/policy/agent-comment.ss",
    "src/policy/agent-dependency-adapter.ss",
    "src/policy/agent-import.ss",
    "src/policy/agent-list-growth.ss",
    "src/policy/agent-list-random-access.ss",
    "src/policy/agent-macro-io.ss",
    "src/policy/agent-macro-protocol.ss",
    "src/policy/agent-package-build-system.ss",
    "src/policy/agent-poo-callees.ss",
    "src/policy/agent-poo-loop-performance.ss",
    "src/policy/agent-poo-object-literal.ss",
    "src/policy/agent-poo.ss",
    "src/policy/agent-source-scope.ss",
    "src/policy/agent-string-growth.ss",
    "src/policy/agent-style-destructuring-signals.ss",
    "src/policy/agent-style-details.ss",
    "src/policy/agent-style-docs.ss",
    "src/policy/agent-style-gerbil-boundary-signals.ss",
    "src/policy/agent-style-gerbil-macro-signals.ss",
    "src/policy/agent-style-gerbil-signal-support.ss",
    "src/policy/agent-style-gerbil-signals.ss",
    "src/policy/agent-style-message.ss",
    "src/policy/agent-style-performance-signals.ss",
    "src/policy/agent-style-quality.ss",
    "src/policy/agent-style-shape.ss",
    "src/policy/agent-style-steering.ss",
    "src/policy/agent-style.ss",
    "src/policy/agent-support.ss",
    "src/policy/agent.ss",
    "src/policy/catalog.ss",
    "src/policy/core.ss",
    "src/policy/dependency-adapter-profile.ss",
    "src/policy/detection.ss",
    "src/policy/facade.ss",
    "src/policy/gerbil-utils-source.ss",
    "src/policy/gxtest-report.ss",
    "src/policy/gxtest.ss",
    "src/policy/model.ss",
    "src/policy/modularity.ss",
    "src/policy/poo-source.ss",
    "src/policy/prototype.ss",
    "src/policy/repair.ss",
    "src/support/time.ss",
    "src/types/core.ss",
    "src/types/env.ss",
    "src/types/facade.ss",
    "src/types/findings.ss",
    "src/types/model.ss",
    "src/types/signatures.ss",
    "src/types/source-findings.ss",
    "src/types/subtyping.ss",
    "src/types/validation.ss",
    "src/utilities/contract-syntax.ss",
    "src/utilities/contracts.ss",
    "src/utilities/functional.ss",
    "src/utilities/projection.ss",
]
