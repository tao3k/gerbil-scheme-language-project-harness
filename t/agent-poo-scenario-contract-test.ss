;;; -*- Gerbil -*-
;;; Top-level smoke entry for fast POO scenario contracts.
...
(def agent-poo-scenario-contract-test
  (test-suite "gerbil scheme harness agent POO scenario smoke"
    (test-case "POO performance scenarios own benchmark files"
      (check (missing-scenario-benchmarks +poo-performance-scenario-ids+)
             => []))

    (test-case "representative POO scenario is covered by native POO registry"
