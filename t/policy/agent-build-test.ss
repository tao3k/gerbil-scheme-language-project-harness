...
(def agent-build-policy-test
  (test-suite "gerbil scheme harness agent build policy"
          ...
          (let ((root ".run/policy-package-build-shell-pipeline"))
            ...
            (let* ((index (collect-project root))
              ...
              (check (not (not (member "shell-pipeline-literal" groups)))
                     => #t))))
    (test-case "agent policy rejects package build without native build surface"
    ...
    (test-case "agent policy accepts defbuild-script package build"
          (let ((root ".run/policy-package-build-canonical-defbuild"))
            ...
            (let* ((index (collect-project root))
                   ...
                   (matching (filter-rule "GERBIL-SCHEME-AGENT-R025" findings)))
              (check matching => []))))
    (test-case "agent policy accepts apply make intermediate package build"
