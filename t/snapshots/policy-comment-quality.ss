(policyScenario
 ...
 (before
  ...
  (comment
   ((styleGuide "engineering-comment-quality")
    ...
    (weakCommentCount 1)
    (repairTargets ("order-totals"))
    (exampleTarget "order-totals")
    (exampleContext "higher-order")
    (exampleCommentKind "contract-only")
    (exampleQuality "weak")
    (exampleReasons ("contract-only-is-not-engineering-comment")))))
 (after
  (r015Findings ())))
