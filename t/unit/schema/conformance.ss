...
(def (check-runtime-source-json-schema-conformance)
  (let* ((packet (search-json ["runtime-source" "writeenv" "printer" "hook" "--json" "."]))
         ...
         (selector-resolver (hash-get packet "selectorResolver"))
         (source-examples (hash-get packet "sourceExamples"))
         (source-comments (hash-get packet "sourceComments"))
    ...
    (check (hash-get selector-resolver "output") => "code-with-comments")
