  ...
  (test-suite "gerbil scheme harness policy"
    ...
    (test-case "modularity policy rejects legacy test directory layout"
      (let* ((root ".run/policy-test-directory-layout")
        ...
        (check (length matching) => 1)
        (check (type-finding-rule-id finding)
               => "GERBIL-SCHEME-MOD-R006")
        (check (type-finding-path finding) => "test/bad-test.ss")
        (check (type-finding-message finding)
               => "Gerbil unit test owner test/bad-test.ss uses legacy test/ layout; use t/ while keeping unit tests explicit")))
...
(def (write-test-directory-layout-project root)
