  ...
  (test-suite "gerbil scheme harness policy"
    ...
    (test-case "agent policy requires POO method generic and class facts"
      (let* ((root ".run/policy-poo-method-shape")
        ...
        (check (type-finding-message finding)
               => "POO method order-discount is missing parser-owned defgeneric,defclass facts; query POO pattern evidence and add defgeneric/defclass structure before extending methods")))
