  ...
  (test-suite "gerbil scheme harness search"
    ...
    (test-case "runtime-source json uses schema-backed acquisition packet"
      (let* ((output (search-output ["runtime-source" "macro" "--json" "."]))
             ...
             (packet-resolver (hash-get packet "selectorResolver"))
             (packet-examples (hash-get packet "sourceExamples"))
             (packet-comments (hash-get packet "sourceComments"))
        ...
        (check (hash-get packet-resolver "scheme") => "gerbil-runtime-source")
        (check (hash-get (car packet-examples) "id") => "std-sugar-defrule")
        (check (hash-get (car packet-comments) "fallback") => "comment-missing-is-signal")
