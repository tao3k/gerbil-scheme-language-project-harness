...
(def (poo-extension-active? index)
  (let (package (project-index-package index))
    (and package
         (or (poo-package-token? (project-package-name package))
             (ormap poo-package-token? (project-package-dependencies package))))))
...
(def (poo-extension-summary-line index)
  (let (package (project-index-package index))
    (string-append
     "|extension name=poo activation=gerbil.pkg packageManager=gxpkg package="
