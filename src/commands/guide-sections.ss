...
(import :gslph/src/policy/catalog (only-in :std/misc/list unique))
...
(def +guide-policy-section+
     (make-guide-section
      "policy"
      (append ["|policy structural-json-boundary=search structural --json emits a lightweight ASP-owned index interface; use --owner <path> for owner-bounded native facts and --artifact only for explicit validation"
